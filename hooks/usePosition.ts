...
const usePosition = (positionId: number, refresh: boolean) => {
  ...
  const [underlyingData, setUnderlyingData] = useState<{tokens:string[], amounts:BigNumber[], values: BigNumber[]}|undefined>()
  const [rewardData, setRewardData] = useState<{tokens:string[], amounts:BigNumber[], values: BigNumber[]}|undefined>()
  ...
  useEffect(() => {
    const fetchUnderlying = async (token:string, index:number) => {
      ...
      return {
        ...
        balance: underlyingData.amounts[index].toString(), formattedBalance: amount, underlying: [], protocol_name: '',
  ...
  useEffect(() => {
    const fetchRewards = async (token:string, index:number) => {
      ...
      return {
        contract_name: name, contract_address: token, contract_ticker_symbol: symbol,
        contract_decimals: decimals, quote: usdValue, quote_rate: 0, chain_id: chainId,
        balance: rewardData.amounts[index].toString(), formattedBalance: amount, underlying: [], protocol_name: '',
