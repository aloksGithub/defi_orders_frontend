...
const useUniversalSwapGetAmounts = (
  ...
  const {contracts, slippageControl: {slippage}} = useAppContext()
  const [loadingText, setLoadingText] = useState('')
  const [loading, setLoading] = useState(false)
  const {data:swapData, loading: childLoading, loadingText: preSwapLoadingText} = useUniversalSwapPreSwap({assetsToConvert, wantedAssets, parentLoading, triggerError})
  ...
  useEffect(() => {
    ...
    const getAmountsOut = async () => {
      const {amounts, expectedUSDValues} = await contracts.universalSwap.getAmountsOutWithSwaps(swapData.provided, swapData.desired, swapData.swaps, swapData.conversions)
  ...
  return {data: {...swapData, expectedAssets}, loading: childLoading || loading, loadingText: loadingText || preSwapLoadingText}
}
