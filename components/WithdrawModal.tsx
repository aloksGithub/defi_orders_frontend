import { useColorModeValue, Flex, NumberInput, NumberInputField, Slider, SliderTrack, SliderFilledTrack, SliderThumb, Box, Text, useDisclosure, Modal, ModalBody, ModalCloseButton, ModalContent, ModalHeader, ModalOverlay } from "@chakra-ui/react"
...
const WithdrawModal = ({position, refreshData, closeSelf}: {position: FetchPositionData, refreshData:Function, closeSelf: Function}) => {
  ...
  const closePosition = () => {
    setClosing(true)
    close(contracts, position.positionId).then((hash)=>{
      setClosing(false)
      successModal("Withdrawal Successful", 
        <Text>
          Assets were withdrawn successfully, 
  ...
  return (
    <Box>
      ...
      <PrimaryButton isLoading={isWithdrawing} size='large' onClick={withdrawFromPostion}>Withdraw</PrimaryButton>
